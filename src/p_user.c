...
void P_SpawnSpinMobj(player_t *player, mobjtype_t type)
{
	...
	if (type == MT_GHOST)
		mobj = P_SpawnGhostMobj(player->mo); // virtually does everything here for us
	else if (type == MT_THOK) {
		mobj = P_SpawnMobjFromMobj(player->mo, 0, 0, 0, type);
		mobj->angle = player->drawangle;
		mobj->color = player->mo->color;
		mobj->skin = player->mo->skin;
		mobj->destscale = 0;
		mobj->scalespeed = FixedMul(mobj->scalespeed, mobj->scale);
...
boolean P_PlayerHitFloor(player_t *player, boolean dorollstuff)
{
	...
	if ((clipmomz = !(P_CheckDeathPitCollide(player->mo))) && player->mo->health) //&& !player->spectator)
	{
		...
		else
		{
			...
			else if (player->pflags & PF_GLIDING) // glide landing
			{
				if (dorollstuff)
				{
					if (player->cmd.buttons & BT_SPIN && player->mo->eflags & MFE_JUSTHITFLOOR && cv_knucklesfix.value) 
					{	
						player->pflags &= ~PF_GLIDING;
						P_SetPlayerMobjState(player->mo, S_PLAY_ROLL);
...
#define chargecalculation (6*(player->dashspeed - player->mindash))/(player->maxdash - player->mindash)
						fixed_t soundcalculation = chargecalculation; // astro come back here for ez spindash!!!
...
static void P_DoJumpStuff(player_t *player, ticcmd_t *cmd)
{
					...
					if (!(player->pflags & PF_THOKKED) || player->charflags & SF_MULTIABILITY)
					{
						...
						if (player->mo->eflags & MFE_UNDERWATER)
						{
							...
							if (!cv_knucklesfix.value)
								...
								playerspeed = 6*playerspeed/7;
							if (cv_knucklesfix.value) {
								player->mo->momx = (6*(player->mo->momx - player->cmomx)/7) + player->cmomx;
								player->mo->momy = (6*(player->mo->momy - player->cmomy)/7) + player->cmomy;
							}
							else if (!(player->powers[pw_super] || player->powers[pw_sneakers]) && !cv_knucklesfix.value)
							{
	...
	else // If not pressing the jump button
	{
		...
		// If letting go of the jump button while still on ascent, cut the jump height. <- astro this is where you left off!!!
		if (((player->pflags & (PF_JUMPED|PF_STARTJUMP)) == (PF_JUMPED|PF_STARTJUMP)) && (P_MobjFlip(player->mo)*player->mo->momz > 0))
		{
			player->mo->momz >>= 1; // divde by 2
			player->pflags &= ~PF_STARTJUMP;
		}
	}
}
...
static void P_3dMovement(player_t *player)
{
	...
	if (player->climbing)
	{
		if (cmd->forwardmove)
		{
			if (!cv_knucklesfix.value)
				...
				if (player->powers[pw_super])
					P_SetObjectMomZ(player->mo, FixedDiv(cmd->forwardmove*FRACUNIT, 5*FRACUNIT), false); // 2/3 while super
				else 
					P_SetObjectMomZ(player->mo, FixedDiv(cmd->forwardmove*FRACUNIT, player->powers[pw_sneakers] ? 6*FRACUNIT : 8*FRACUNIT), false);
		}
	}
	...
	if (player->climbing)
	{
		if (!cv_knucklesfix.value) 
			...
			if (player->powers[pw_super]) 
				P_InstaThrust(player->mo, player->mo->angle-ANGLE_90, FixedDiv(cmd->sidemove*player->mo->scale, 5*FRACUNIT)); // 2/3 while super
			else 
				P_InstaThrust(player->mo, player->mo->angle-ANGLE_90, FixedDiv(cmd->sidemove*player->mo->scale, player->powers[pw_sneakers] ? 6*FRACUNIT : 8*FRACUNIT));
	}
...
void P_MovePlayer(player_t *player)
{
	...
	if (player->charability == CA_GLIDEANDCLIMB && player->mo->state-states == S_PLAY_GLIDE_LANDING && !cv_knucklesfix.value)
	{
		player->pflags |= PF_STASIS;
	}
...
//////////////////
	...
	if (player->pflags & PF_GLIDING)
	{
		...
		if (!player->skidtime) // TODO: make sure this works in 2D!
		{
			...
			if (cv_knucklesfix.value)
				accelfactor = 13*FRACUNIT/2 - 3*FINECOSINE((anglediff >> ANGLETOFINESHIFT) & FINEMASK);
			else
				accelfactor = 4*FRACUNIT - 3*FINECOSINE((anglediff >> ANGLETOFINESHIFT) & FINEMASK);
	...
	if (player->pflags & PF_SPINNING && P_AproxDistance(player->speed, player->mo->momz) > FixedMul(15<<FRACBITS, player->mo->scale))
	{
